Rails.application.routes.draw do
  ...
  post("/insert_course", { :controller => "courses", :action => "create" }) # Changed from get to post
  post("/modify_course/:path_id", { :controller => "courses", :action => "update" }) # Changed from get to post
  ...
  post("/delete_student/:path_id", { :controller => "students", :action => "destroy" })

  # Enrollment route
  post("/insert_enrollment", { :controller => "enrollments", :action => "create" })

  # You can remove any other duplicate or incorrectly defined routes.
end
